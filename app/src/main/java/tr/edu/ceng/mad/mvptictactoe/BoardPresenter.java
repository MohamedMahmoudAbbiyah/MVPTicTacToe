...
import static tr.edu.ceng.mad.mvptictactoe.BoardView.PLAYER_2_WINNER;
...
public class BoardPresenter implements BoardListener {
    ...
    public BoardPresenter(BoardView boardView) {
        view = boardView;
        board = new Board((BoardListener) this);
    }
    ...
    public void gameEnded(byte winner) {
            switch (winner){
                ...
                case BoardListener.PLAYER_2:
                    view.gameEnded(PLAYER_2_WINNER);
    ...
    @Override
    public void invalidPlay(byte player, byte row, byte col) {
